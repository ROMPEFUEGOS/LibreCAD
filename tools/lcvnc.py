...
class LC:
    def __init__(self, variant, port, confdir, shots_dir, args=()):
        ...
        # 2.2.1 stores its settings in LibreCAD.conf; master in LibreCAD-<schema>.conf
        conf = f"{confdir}/config/LibreCAD/{os.environ.get('LC_CONF_FILE', 'LibreCAD.conf')}"
        if not os.path.exists(conf):
            with open(conf, "w") as f:
                f.write("[Startup]\nFirstLoad=0\nShowSplash=false\nCheckForNewVersions=false\n\n"
                        "[Appearance]\nLanguage=en\nLanguageCmd=en\n\n"
                        "[Geometry]\nWindowWidth=1700\nWindowHeight=1300\nWindowX=0\nWindowY=0\n")
        # refuse to start if something already listens on the port: the client would
        # silently drive that other instance instead of ours
        try:
            with socket.create_connection(("127.0.0.1", port), timeout=0.5):
                raise RuntimeError(f"port {port} is already in use (stale librecad?)")
        except OSError:
            pass
        runner = os.environ.get("LC_RUNNER", f"{TOOLS}/run_lc.sh")
        self.proc = subprocess.Popen([runner, variant, str(port), confdir, *args],
    ...
    def close(self):
        ...
        # vncdotool's Twisted reactor cannot be restarted in the same process:
        # multi-instance drivers set LC_NO_SHUTDOWN=1 and call api.shutdown() once at exit.
        if os.environ.get("LC_NO_SHUTDOWN") != "1":
            try: api.shutdown()
            except Exception: pass
