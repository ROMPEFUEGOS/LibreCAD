"""GUI scenarios for the HIDDEN linetype family on LibreCAD master (Qt vnc platform).

Usage: LC_RUNNER=tools/run_lc_master.sh LC_CONF_FILE=LibreCAD-2.conf \
       tools/venv/bin/python tools/lc_gui_test_hidden.py <build: base|hidden> <port> <out-dir>

Scenarios (each one is a fresh LibreCAD instance with an isolated config):
  G1  pen toolbar line-type list          -> shot of the open combobox
  G2  pick "Hidden", draw a line, Save As -> saved DXF checked by name + LTYPE table
  ...
  G5  open hidden_family.dxf, Save As     -> resaved DXF checked (names kept or lost)
...
FIXTURES = os.path.join(TOOLS, "..", "test-results", "hidden", "fixtures")
...
class Session:
    ...
    def open_linetype_box(self, name, steps=11):
        ...
        highlight `steps` rows down so the entries after "Dash (large)" scroll
        into view. Esc leaves the current value untouched."""
...
def main():
    ...
    if wanted("G1"):
        ...
        try:
            results["scenarios"]["G1_combobox_list"] = s.open_linetype_box("combo_open", steps=11)
        finally:
            s.close()
    if wanted("G1F"):
        s = Session(build, port, out, "g1f")
        try:
            results["scenarios"]["G1F_combobox_family"] = s.open_linetype_box("combo_open_family", steps=14)
        finally:
            s.close()

    saved = os.path.join(out, "hidden_saved.dxf")
    if wanted("G2"):
        s = Session(build, port, out, "g2")
        try:
            sel = s.set_pen_linetype("hidden")
            s.draw_line()
            s.save_as(saved)
            results["scenarios"]["G2_pick_hidden_draw_save"] = {
                "combobox_crop": sel, "dxf": saved,
                "check": check_dxf(saved, ["HIDDEN"])}
        finally:
            s.close()
    ...
    if wanted("G4"):
        g4 = {}
        for name in ("HIDDEN", "HIDDEN2", "HIDDENX2"):
    ...
    if wanted("G5"):
        fx = os.path.join(FIXTURES, "hidden_family.dxf")
        s = Session(build, port, out, "g5", args=(fx,))
        try:
            resaved = os.path.join(out, "hidden_family_resaved.dxf")
            s.save_as(resaved)
            results["scenarios"]["G5_fixture_resaved"] = {
                "dxf": resaved, "check": check_dxf(resaved, ["HIDDEN", "HIDDEN2", "HIDDENX2"])}
        finally:
            s.close()
