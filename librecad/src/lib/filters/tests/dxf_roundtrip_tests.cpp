...
TEST_CASE("DXF round-trip of a custom linetype is stable in both DXF versions",
  ...
  for (const auto &[version, label] :
    ...
    const std::string out2 =
        tmpFile(r12 ? "named_t2_out12_then2007.dxf" : "named_t2_out_again.dxf");
    std::filesystem::remove(out);
    std::filesystem::remove(out2);
    ...
    // A second export writes the same record and references, once each. An
    // R12 file with an INSERT does not export to R12 again yet, names or not,
    // so that lane exports the second time to R2007.
    {
      RS_FilterDXFRW filter;
      REQUIRE(filter.fileExport(reimported, QString::fromStdString(out2),
                                r12 ? RS2::FormatDXFRW : version));
    }
...
TEST_CASE("DXF complex linetype record travels through export opaque",
  ...
  // The 340 style handle is not checked: master already writes it pointing at
  // a handle the STYLE table does not mint.
...
TEST_CASE("DXF a linetype named only by an MLINESTYLE element reaches the walk",
  ...
  // R2000: this OBJECTS section does not export above it, names or not.
  {
    RS_FilterDXFRW filter;
    REQUIRE(filter.fileExport(graphic, QString::fromStdString(out),
                              RS2::FormatDXFRW2000));
